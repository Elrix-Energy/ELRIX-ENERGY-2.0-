...
import { Sun, ShieldCheck, Wrench, IndianRupee, ArrowRight, Calculator, Star, ChevronDown, ChevronUp, MapPin } from 'lucide-react';
...
const Home = () => {
  ...
  const calculateSolar = (e) => {
    ...
    const kwSize = numBill / 1100;
    const requiredKw = kwSize.toFixed(1);
    const savings = (numBill * 12 * 25).toLocaleString('en-IN');
    
    // Cost & Subsidy Estimation
    const grossCost = kwSize * 65000;
    let subsidy = 0;
    if (kwSize >= 3) subsidy = 78000;
    else if (kwSize >= 2) subsidy = 60000;
    else if (kwSize >= 1) subsidy = 30000;
    else subsidy = kwSize * 30000; // Partial kW

    const netCost = Math.max(0, grossCost - subsidy);
    const formattedNetCost = Math.round(netCost).toLocaleString('en-IN');

    // Environmental impact estimates
    const co2Offset = (Number(requiredKw) * 31).toFixed(1);
    const trees = Math.round(Number(co2Offset) * 40).toLocaleString('en-IN');

    setCalcResult({
      systemSize: requiredKw,
      lifetimeSavings: savings,
      estimatedNetCost: formattedNetCost,
      co2: co2Offset,
      treesPlanted: trees
    });
  };
    ...
    <div className="home">
        ...
        <div className="container hero-content">
          <Reveal delay={0}>
            ...
            <div className="hero-cta-group">
              <Link href="/contact" className="btn btn-hero-primary">Get Free Quote</Link>
              <a href="tel:+919640484677" className="btn btn-hero-outline">Call Now: +91 9640484677</a>
            </div>
          </Reveal>
        </div>
      ...
      <div className="trust-strip">
        <div className="container trust-strip-inner">
          <div className="trust-item">✓ MNRE Certified</div>
          <div className="trust-item">✓ APSPDCL Empanelled</div>
          <div className="trust-item">✓ ISO 9001:2015</div>
          <div className="trust-item">✓ MSME Registered</div>
        </div>
      </div>
      ...
      {/* Service Areas Strip */}
      <section className="service-areas-strip">
        <div className="container">
          <h3 className="areas-heading">Proudly Serving Andhra Pradesh</h3>
          <div className="areas-grid">
            {serviceAreas.map((area) => (
              <div className="area-item" key={area.city}>
                <MapPin size={20} className="area-icon" />
                <span className="area-city">{area.city}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {serviceCards.map((service, index) => (
              <Reveal delay={index * 150} key={service.title}>
                <div className="service-card glass">
                  ...
                  <Link href={service.link} className="text-secondary font-bold">Learn More &rarr;</Link>
                </div>
              </Reveal>
            ))}
          </div>
                ...
                {calcResult && (
                  <>
                    <div className="calc-result">
                      ...
                      <div className="result-box">
                        <h4>Est. Net Cost</h4>
                        <p className="result-value font-bold" style={{ fontSize: '1.5rem' }}>₹ {calcResult.estimatedNetCost}</p>
                      </div>
                      <div className="result-box">
                    ...
                    <div className="calc-lead-capture">
                      <p>A <strong>{calcResult.systemSize} kW</strong> system pays for itself quickly. Invest ~<strong>₹ {calcResult.estimatedNetCost}</strong> (after subsidy) to save <strong>₹ {calcResult.lifetimeSavings}</strong> over 25 years.</p>
                      <div className="calc-cta-group">
                        <Link href={`/contact?bill=${bill}`} className="btn btn-primary">Get Detailed Quote</Link>
                        <a
                          href={`https://wa.me/919640484677?text=${encodeURIComponent(`Hi, I calculated my solar savings on your website.\nMy monthly bill: ₹${bill}\nRecommended system: ${calcResult.systemSize} kW\nEst. 25-year savings: ₹${calcResult.lifetimeSavings}\n\nPlease share a detailed quote.`)}`}
                          className="btn btn-outline"
                          target="_blank"
                          rel="noreferrer"
                        >WhatsApp This Estimate</a>
                      </div>
                    </div>
                  </>
                )}
        ...
        <div className="container">
          <Reveal>
            <div className="experience-card text-center">
              <Star size={48} className="experience-icon" />
              <h2>Your Experience Matters to Us</h2>
              <p style={{ maxWidth: '600px', margin: '1rem auto 2rem', color: 'var(--text-light)', fontSize: '1.1rem', lineHeight: '1.7' }}>
                We are building our reputation one rooftop at a time across Nellore, Tirupati, Kadapa, and Ongole. Be among the first to share your ELRIX ENERGY solar journey.
              </p>
              <Link href="/contact" className="btn btn-outline">Share Your Experience →</Link>
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="section bg-background">
        <div className="container">
          <div className="flex-between mb-3">
            ...
            <Link href="/blog" className="btn btn-outline" style={{ padding: '0.5rem 1rem' }}>View All Blogs</Link>
          </div>
            ...
            {blogPreviews.map((post, index) => (
              <Reveal delay={index * 150} key={post.title}>
                <div className="blog-card bg-white">
                  ...
                  <Link href="/blog" className="text-primary font-bold mt-1 inline-block">Read More &rarr;</Link>
                </div>
              </Reveal>
            ))}
          </div>
